...
namespace PrimitivesPro.Editor
{
    public class Toolbar : EditorWindow
    {
        ...
        private void GridCallback(object obj)
        {
            var gridOptions = (GridOptions) obj;

            switch (gridOptions)
            {
                case GridOptions.Enable:
                    ShowGrid();
                    break;

                case GridOptions.Settings:
                    ShowGridSettings();
                    break;
            }
        }
        ...
        private void ShowGridSettings()
        {
            ...
            if (meshEditor != null && meshEditor.IsValid(selectedObject))
            {
                GridSettings.ShowWindow(meshEditor.GetGrid());
            }
        }
